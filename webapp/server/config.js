...
const config = {
  ...
  SLURM: {
    ...
    JOBS_INPUT_MAX_SIZE_BYTES:
      makeIntIfDefined(process.env.SLURM_JOBS_INPUT_MAX_SIZE_BYTES) ||
      161061273600
  },
  NEXTFLOW: {
    ...
    TEMPLATE_DIR:
      process.env.NEXTFLOW_TEMPLATE_DIR ||
      path.join(NEXTFLOW_BASE_DIR, 'templates'),
    // Directory of the workflow configs. The Workflow configs are used for running nextflow workflows.
    CONFIG_DIR:
      process.env.NEXTFLOW_CONF_DIR || path.join(NEXTFLOW_BASE_DIR, 'configs')
  },
  CROMWELL: {
    ...
    WDL_DIR:
      process.env.CROMWELL_WDL_DIR || path.join(CROMWELL_BASE_DIR, 'WDL'),
    // Directory of the workflow templates. The Workflow templates are used for creating cromwell inputs.
    TEMPLATE_DIR:
      process.env.CROMWELL_TEMPLATE_DIR ||
      path.join(CROMWELL_BASE_DIR, 'templates'),
  ...
  CRON: {
    ...
    SCHEDULES: {
      ...
      SLURM_WORKFLOW_MONITOR:
        process.env.CRON_SLURM_WORKFLOW_MONITOR_SCHEDULE || '0-59/3 * * * *',
      // monitor nextflow jobs on every 3 minutes
      SLURM_JOB_MONITOR:
        process.env.CRON_SLURM_JOB_MONITOR_SCHEDULE || '1-59/3 * * * *',
      ...
      TRAME_PUBLIC_MONITOR:
        process.env.CRON_SLURM_JOB_MONITOR_SCHEDULE || '2-59/3 * * * *'
    }
  },
  DATABASE: {
  ...
  EMAIL: {
    ...
    ACTIVATE_USER_SUBJECT:
      process.env.ACTIVATE_USER_SUBJECT || 'Your EPIC EDGE login account',
    ...
    RESETPASSWORD_SUBJECT:
      process.env.RESETPASSWORD_SUBJECT ||
      'Reset your EPIC EDGE login password',
    ...
    // project status
    SEND_PROJECT_STATUS_EMAILS:
      makeBoolean(process.env.SEND_PROJECT_STATUS_EMAILS) || false,
    PROJECT_STATUS_SUBJECT:
      process.env.PROJECT_STATUS_SUBJECT || 'Your EPIC EDGE project status'
  },
  FILE_UPLOADS: {
  ...
  EPIC: {
    ...
    PRODUCT_BASE_DIR:
      process.env.EPIC_PRODUCT_BASE_DIR || path.join(IO_BASE_DIR, 'products'),
    // Directory to store structure datasets
    STRUCTURE_BASE_DIR:
      process.env.EPIC_STRUCTURE_BASE_DIR ||
      path.join(IO_BASE_DIR, 'structures'),
    ...
    PVPYTHON_PLUGIN_DIR:
      process.env.PVPYTHON_PLUGIN_DIR || 'PVPYTHON_PLUGIN_DIR-not-found-in-env',
    ...
    COMPARE_SETTINGS:
      process.env.COMPARE_SETTINGS || 'COMPARE_SETTINGS-not-found-in-env',
    COMPARE_ENV_PYTHONPATH:
      process.env.COMPARE_ENV_PYTHONPATH ||
      'COMPARE_ENV_PYTHONPATH-not-found-in-env'
  }
}
