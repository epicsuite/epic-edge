...
const getStructure = (code, type, user) =>
  new Promise((resolve, reject) => {
    ...
    Structure.findOne({
      ...
      .then(structure => {
        ...
        if (
          type === 'user' &&
          (structure.owner === user.email ||
            structure.sharedTo.includes(user.email) ||
            structure.public)
        ) {
          resolve(structure)
...
const updateStructure = (query, req) =>
  new Promise((resolve, reject) => {
    Structure.findOne(query)
      .then(oldStructure => {
