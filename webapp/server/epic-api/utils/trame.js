...
const startTrame = async (req, res, type) => {
  try {
    ...
    let port =
      type === 'user'
        ? config.EPIC.TRAME_USER_PORT_START
        : config.EPIC.TRAME_PUBLIC_PORT_START
    ...
    if (
      port >
      (type === 'user'
        ? config.EPIC.TRAME_USER_PORT_END
        : config.EPIC.TRAME_PUBLIC_PORT_END)
    ) {
      return res.status(400).json({
        success: false,
    ...
    if (trameApp === 'structure') {
      ...
      await fs.promises.copyFile(
        config.EPIC.COMPARE_SETTINGS,
        `${trameHome}/settings.yaml`
      )
