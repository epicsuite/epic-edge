...
const validationRules = () => [
  ...
  body('notification')
    .optional()
    .isBoolean()
    .withMessage('Invalid notification. Must be true or false.'),
  body('notificationEmail')
    .optional()
    .trim()
    .normalizeEmail()
    .isEmail()
    .withMessage('Invalid notificationEmail address.'),
  body('jobLimit')
    .optional()
    .trim()
    .isInt({ min: 1 })
    .withMessage('Invalid job limit. Must be an integer number and > 0.'),
  body('jobPriority')
    .optional()
    .trim()
    .isInt()
    .withMessage('Invalid job priority. Must be an integer number.'),
  body('role')
    .optional()
    .trim()
    .isIn(['user', 'admin'])
