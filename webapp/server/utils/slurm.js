...
const generateInputs = async (projHome, projectConf, proj) => {
  ...
  for (let i = 0; i < jsonArray.length; i += 1) {
    ...
    const file1 = await Upload.findOne({
      folder: { $eq: folder1 },
      name: { $eq: upload1 },
      status: { $ne: 'delete' }
    })
    if (!file1) {
      ...
      // eslint-disable-next-line no-await-in-loop
      const fqlink1 = await linkUpload(
        `${config.IO.UPLOADED_FILES_DIR}/${file1.code}`,
        projHome
      )
      fq1s.push(fqlink1)
      newCsv += `${fqlink1},`
    ...
    // find uploaded file
    // eslint-disable-next-line no-await-in-loop
    const file2 = await Upload.findOne({
      folder: { $eq: folder2 },
      name: { $eq: upload2 },
      status: { $ne: 'delete' }
    })
    if (!file2) {
      ...
      // add file link to input directory
      // eslint-disable-next-line no-await-in-loop
      const fqlink2 = await linkUpload(
        `${config.IO.UPLOADED_FILES_DIR}/${file2.code}`,
        projHome
      )
      fq2s.push(fqlink2)
      newCsv += `${fqlink2},`
  ...
  const conf = {
    ...
    timevalues: projectConf.workflow.input.workflowInput.timeValues
      .split(/,\s*/)
      .map(str => parseInt(str, 10)),
    treatments:
      projectConf.workflow.input.workflowInput.treatments.split(/,\s*/),
    fqs: [fq1s, fq2s]
  }
  // generate workflow.init file
  const workflowInitTemplate = `cell_line:    <%= cellLine %>
...
const submitWorkflow = async (proj, projectConf, inputsize) => {
  ...
  newJob.save().catch(err => {
    logger.error('falied to save to slurmjob: ', err)
  })
  proj.status = 'running'
  proj.updated = Date.now()
  proj.save()
