...
function AddData(props) {
  ...
  const onSubmit = () => {
    ...
    //annotations
    if (
      modules['annotations'].paramsOn &&
      (modules['annotations'].genesFile || modules['annotations'].featuresFile)
    ) {
      myWorkflow.annotations = {}
      inputDisplay['annotations'] = {}
      if (modules['annotations'].genesFile) {
        myWorkflow.annotations.genes = {
          file: modules['annotations'].genesFile,
          description: modules['annotations'].genesDescription,
        }
        inputDisplay['annotations'].genes = {
          file: modules['annotations'].genesFileDisplay,
          description: modules['annotations'].genesDescription,
        }
      }
      if (modules['annotations'].featuresFile) {
        myWorkflow.annotations.features = {
          file: modules['annotations'].featuresFile,
          description: modules['annotations'].featuresDescription,
        }
        inputDisplay['annotations'].features = {
          file: modules['annotations'].featuresFileDisplay,
          description: modules['annotations'].featuresDescription,
        }
      }
    }
  ...
  useEffect(() => {
    ...
    //no data
    if (
      modules['tracks'].tracks.length === 0 &&
      !modules['annotations'].genesFile &&
      !modules['annotations'].featuresFile &&
      modules['bookmarks'].locations.length === 0 &&
      modules['bookmarks'].features.length === 0
    ) {
      setRequestSubmit(false)
    }
              ...
              >
                <div className="clearfix">
                  {initial4DGB['tracks'].process.on && (
                    <Tracks
                      title={initial4DGB['tracks'].process.full_name}
                      name={'tracks'}
                      workflow={workflow}
                      isValid={modules['tracks'].validForm}
                      errMessage={modules['tracks'].errMessage}
                      setParams={setModuleParams}
                    />
                  )}
                  {initial4DGB['annotations'].process.on && (
                    <Annotations
                      title={initial4DGB['annotations'].process.full_name}
                      name={'annotations'}
                      workflow={workflow}
                      isValid={modules['annotations'].validForm}
                      errMessage={modules['annotations'].errMessage}
                      setParams={setModuleParams}
                    />
                  )}
                  {initial4DGB['bookmarks'].process.on && (
                    <Bookmarks
                      title={initial4DGB['bookmarks'].process.full_name}
                      name={'bookmarks'}
                      workflow={workflow}
                      isValid={modules['bookmarks'].validForm}
                      errMessage={modules['bookmarks'].errMessage}
                      setParams={setModuleParams}
                    />
                  )}
                  <br></br>
