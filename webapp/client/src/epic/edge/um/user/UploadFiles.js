...
import {
  ...
  getFileExtension,
  apis,
} from 'src/edge/common/util'
import 'react-toastify/dist/ReactToastify.css'

import { LoaderDialog } from 'src/edge/common/Dialogs'
import { MyCreatableSelect } from 'src/edge/common/MySelect'
    ...
    getData(apis.uploadsInfo)
      .then((data) => {
        ...
        setAllowedExtensions(
          props.extensions ? props.extensions : data.allowedExtensions.split('|'),
        )
        setAccept(props.extensions ? `.${props.extensions.join(', .')}` : '*')
      })
      .catch((err) => {
        alert(err)
      })
  ...
  return (
    <>
      ...
      <div className="clearfix">
        {props.header && <h4 className="pt-3">{props.header}</h4>}
        {props.info && (
          <span className="pt-3 text-muted edge-text-size-small">
            <HtmlText text={props.info} />
            <br></br>
            <br></br>
          </span>
        )}
