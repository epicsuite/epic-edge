        ...
        {props.data.map((item, ind) => {
          return (
            <tr key={ind}>
              {props.headers.map((header, index) => (
                <td key={index}>
                  {!item[header] || isNaN(Number(item[header])) || item[header].length === 0
                    ? item[header]
        ...
        {props.data.map((item, ind) => {
          return (
            <tr key={ind}>
              {item.map((val, index) => (
                <td key={index}>
                  {!val || isNaN(Number(val)) || val.length === 0
                    ? val
      ...
      <tbody>
        {Object.keys(props.data).map((item, index) => (
          <tr key={index}>
            <td>{item}</td>
            <td>
              {!props.data[item] || isNaN(Number(props.data[item])) || props.data[item].length === 0
                ? props.data[item]
                : Number(props.data[item]).toLocaleString('en-US')}
            </td>
          </tr>
        ))}
      </tbody>
